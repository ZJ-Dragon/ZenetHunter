...
class Wpa3Config(BaseModel):
    ...
    mode: WpaMode = Field(
        default=WpaMode.WPA3_ENTERPRISE,
        description="WPA security mode"
    )
    ssid: str = Field(..., description="Wireless network SSID")
    radius: RadiusConfig | None = Field(
        None,
        description="RADIUS configuration (required for Enterprise mode)"
    )
    eap_method: EapMethod | None = Field(
        None,
        description="EAP method (required for Enterprise mode)"
    )
