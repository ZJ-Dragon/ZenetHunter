...
async def lifespan(app: FastAPI):  # noqa: D401 (docstring optional)
    ...
    from app.core.database import init_db, get_database_url
    from app.core.config import get_settings
    ...
    db_host = "localhost" if settings.database_url and "localhost" in settings.database_url else "unknown"
    
    for attempt in range(max_retries):
        try:
            logger.info(f"Initializing database connection (attempt {attempt + 1}/{max_retries}) to {safe_url}")
            await init_db()
            ...
            try:
                ...
                print(f"Database initialized successfully (logging failed: {log_err})", flush=True)
            break
            ...
            if attempt < max_retries - 1:
                ...
                logger.error(
                    ...
                    f"Database: {safe_url}. "
                    f"Troubleshooting: 1) Check database container: 'docker ps | grep zh-db', "
                    f"2) Check port exposure: 'docker port zh-db', "
                    f"3) Test connection: 'psql -h {db_host} -p 5432 -U zenethunter -d zenethunter', "
            ...
            try:
                ...
                if all_tasks:
                    try:
                        ...
                        logger.warning("Some tasks did not cancel within timeout, continuing shutdown...")
            except Exception as e:
...
if "*" not in cors_origins:
    cors_origins = list(cors_origins) + ["null"]  # "null" is the origin for file:// protocol
