...
class AutoDefenseRequest(BaseModel):
    ...
    max_strategies: int = Field(
        ...
        ge=1,
        le=10,
        description="Maximum number of strategies to apply"
    )
...
class AutoDefenseResponse(BaseModel):
    ...
    strategies_applied: list[str] = Field(..., description="List of applied strategy IDs")
...
async def auto_defense(
    ...
    return AutoDefenseResponse(
        ...
        message=f"Automatically applied {len(applied_strategies)} strategies to device {mac}",
    )
...
class StrategyFeedbackRequest(BaseModel):
    ...
    effect_score: float = Field(..., ge=0.0, le=1.0, description="Effectiveness score (0.0-1.0)")
    resource_cost: float = Field(..., ge=0.0, le=1.0, description="Resource cost (0.0-1.0)")
    duration_seconds: int = Field(default=0, ge=0, description="How long the strategy was active")
...
async def submit_strategy_feedback(
    ...
    try:
        ...
        strategy = StrategyIdentifier(
            type=strategy_type_enum,
            strategy_id=strategy_id_enum
        )
    except (ValueError, KeyError) as e:
        raise AppError(
            ErrorCode.API_BAD_REQUEST,
            f"Invalid strategy type or ID: {e}"
        )
