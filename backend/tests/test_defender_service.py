...
async def test_apply_defense_global_policy_arp_dai():
    ...
    with patch.object(service.arp_monitor, "start_monitoring", new_callable=AsyncMock):
        with patch.object(
            service.engine, "enable_global_protection", new_callable=AsyncMock
        ):
            ...
            service.engine.enable_global_protection.assert_called_once_with(
                DefenseType.ARP_DAI
            )
...
async def test_apply_defense_global_policy_dns_rpz():
    ...
    with patch.object(service.dns_engine, "add_zone", new_callable=AsyncMock):
        with patch.object(service.dns_engine, "add_rule", new_callable=AsyncMock):
            ...
            service.dns_engine.add_rule.assert_called_once_with(
                "malware.test", "NXDOMAIN"
            )
...
async def test_stop_defense_global():
    ...
    # Mock dependencies
    with patch.object(
        service.engine, "disable_global_protection", new_callable=AsyncMock
    ):
        with patch.object(
            service.arp_monitor, "stop_monitoring", new_callable=AsyncMock
        ):
...
async def test_stop_defense_already_inactive():
    ...
    # but the test ensures no exception
