...
示例:
  ...
  python clean-cache.py --docker     # 显示 Docker 清理建议
    ...
    parser.add_argument(
        "--all", action="store_true", help="清理所有缓存（包括数据库和虚拟环境）"
    )
    parser.add_argument("--db", action="store_true", help="同时清理数据库文件")
    parser.add_argument("--venv", action="store_true", help="同时清理虚拟环境")
    parser.add_argument("--docker", action="store_true", help="显示 Docker 清理建议")
    parser.add_argument(
