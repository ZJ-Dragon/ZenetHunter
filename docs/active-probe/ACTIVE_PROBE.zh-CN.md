...
## 概述
主动探测通过模拟正常客户端连接，让设备“自报家门”（厂商、型号、固件）。所有探测均为只读、带超时保护，且**仅对本地/私网 IP 生效**。流程为“服务优先”：先用 mDNS/SSDP 收集服务类型与 TXT 线索，再在存在线索时运行 HTTP/HTTPS 识别与打印机探测（严格超时 + 4 KB 响应上限）。

## 工作原理
引擎会并发尝试多种协议：
1) **HTTP/HTTPS**（`FEATURE_HTTP_IDENT` 控制）：抓取 Web 管理界面（限速 + 4 KB 体积上限）
2) **Telnet**：抓取横幅
3) **SSH**：抓取横幅
4) **打印机协议**（需线索 + `FEATURE_PRINTER_IDENT`）：IPP、LPD
...
- 通过环境变量启用/禁用：
  ...
  FEATURE_HTTP_IDENT=true     # 启用安全 HTTP/HTTPS 识别
  FEATURE_PRINTER_IDENT=true  # 在存在线索时启用打印机识别
  ```
...
- 超时与截断：每个探测有超时，HTTP 响应截断在 4 KB 内。
- 本地限定：仅对私网/回环地址探测，SSDP LOCATION 指向外部时会跳过。
